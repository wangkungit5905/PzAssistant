    ...
    ui(new Ui::BASummaryForm)
{
    ...
    //初始化对方科目列表
    QSqlQuery q;
    bool r = q.exec("select id,subName from FirSubjects where isView = 1");
...
void DetailsViewDialog2::saveExtra()
{
    ...
    s = QString("select tbl_name from sqlite_master where "
                "tbl_name = '%1'").arg(tabName);
    bool r = q.exec(s);
    if(r && !q.first()){
        s = QString("create table %1(id INTEGER PRIMARY KEY, "
                    "year INTEGER, month INTEGER, subId INTEGER, "
                    "extra REAL)").arg(tabName);
        r = q.exec(s);
    }

    //将数据保存到对应表中，保存前检测是否存在老数据，如有则覆盖，否则新增
    int year = ui->dateEdit->date().year();
    int month = ui->dateEdit->date().month();
    int subId;
    if(witch != 4)
        subId = smodel->data(smodel->index(ui->cmbSndSub->currentIndex(), 0)).toInt();
    else
        subId = fmodel->data(fmodel->index(ui->cmbFstSub->currentIndex(), 0)).toInt();

    int col = imodel->columnCount();
    double extra = imodel->data(imodel->index(imodel->rowCount()-1,
                                              col - 1)).toDouble(); //余额值
    s = QString("select id from %1 where (year = %2) and "
                "(month = %3) and (subId = %4)")
            .arg(tabName).arg(year).arg(month).arg(subId);

    if(q.exec(s) && q.first()){  //存在，则覆盖
        s = QString("update %1 set extra = %2 where (year = %3) "
                    "and (month = %4) and (subId = %5)")
                .arg(tabName).arg(extra).arg(year).arg(month).arg(subId);
        r = q.exec(s);
    }
    else{  //没有则插入新的
        s = QString("insert into %1(year, month, subId, extra) "
                    "values(%2, %3, %4, %5)")
                .arg(tabName).arg(year).arg(month).arg(subId).arg(extra);
        r = q.exec(s);
    }*/
...
//    //将模板文件拷贝到用户指定的目录中
//    QString sf = QDir::currentPath().append("/").append("templates/DetailsDialy.xls");
//    int i = 0;
...
//    }
   ...
   //打开目标Excel文档
    ...
    //得到名为cash的一个sheet的指针（现金日记账）
    QAxObject *sheet = sheets->querySubObject("Item(const QVariant&)", sheetName);
    sheet->dynamicCall( "Select()"); //选择这个sheet，使之可见
    QAxObject *cell;

    //现金日记账
    if(witch == 1){
        cell = sheet->querySubObject("Cells(1,1)");
        cell->setProperty("Value", QString(tr("%1年%2月"))
                          .arg(ui->dateEdit->date().year())
                          .arg(ui->dateEdit->date().month()));
        for(int i = 0; i < imodel->rowCount(); ++i)
            for(int j = 0; j < 8; ++j){
                cell = sheet->querySubObject(QString("Cells(%1,%2)")
                                             .arg(i+3).arg(j+1).toLatin1());
                cell->setProperty("Value", imodel->data(imodel->index(i,j)));
            }
    }
    //银行日记账
    if(witch == 2){
        ...
        cell->setProperty("Value", QString(tr("%1年%2月"))
                          ...
                          .arg(ui->dateEdit->date().month()));
        //币种
        QString mts = ui->cmbSndSub->currentText();
        int idx = mts.indexOf("-");
        mts = mts.right(mts.count() - idx - 1);
        cell = sheet->querySubObject("Cells(2,2)");
        cell->setProperty("Value", mts);
        for(int i = 0; i < imodel->rowCount(); ++i)
            for(int j = 0; j < 9; ++j){
                cell = sheet->querySubObject(QString("Cells(%1,%2)")
                                             .arg(i+5).arg(j+1).toLatin1());
                //对于摘要列，只输出摘要说明和引用发票号信息
                if(j == 4){
                    QString summ = imodel->data(imodel->index(i,j)).toString();
                    int idx = summ.indexOf("<");
                    if(idx != -1){
                        int idxs = summ.indexOf("<fp>");
                        int idxe = summ.indexOf("</fp>");
                        if((idxs != -1) && (idxe != -1)){
                            QString s = summ.left(idx);
                            QString s2 = summ.mid(idxs+4, idxe - idxs-4);
                            s.append("(").append(s2).append(")");
