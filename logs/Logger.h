...
// 如下的宏是一些便利的输出日志信息的方法，因为，它使用了可变参数个数的形式，且利用预定义宏
// 可以自动填充输出日志信息的代码所在的文件、代码行号，函数名等信息
...
class Logger
{
  public:
    ...
    enum LogLevel
    {
      Debug   = 1,  //!< Debug level. Useful for non-necessary records used for the debugging of the software.
      Info    = 2,  //!< Info level. Can be used for informational records, which may be interesting for not only developers.
      Warning = 3,  //!< Warning. May be used to log some non-fatal warnings detected by your application.
      Error   = 4,  //!< Error. May be used for a big problems making your application work wrong but not crashing.
      Fatal   = 5,  //!< Fatal. Used for unrecoverable errors, crashes the application right after the log record is written.
      Must    = 6   //!< Must. 无论在什么级别都必须显示，主要用于输出应用程序合适启动和停止等信息
    };

    //! 获取所有支持的日志级别列表
    /**
     *  \param elst 日志级别枚举值列表
     *  \param names 日志结伴名称列表
    */
    static void getLogLevelList(QList<LogLevel>& elst,QList<QString>& names);
    ...
    //! 清空日志信息
    static QList<LogStruct *> clear(LogOutDevice device = File);
};
