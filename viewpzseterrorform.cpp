...
void ViewPzSetErrorForm::viewErrors()
{
    ...
    for(int i = 0; i < es.count(); ++i){
        ...
        item = new QTableWidgetItem(QString::number(pe->pzNum));
        ui->tw->setItem(row,1,item);
        if(pe->baNum == 0)
            item = NULL;
        else
            item = new QTableWidgetItem(QString::number(pe->baNum));
...
{
    if(t->isRunning()){
        ui->lblState->setText(tr("正在检查，请稍候！"));
        return;
    }
    ui->btnInspect->setEnabled(false);
    t->start();
